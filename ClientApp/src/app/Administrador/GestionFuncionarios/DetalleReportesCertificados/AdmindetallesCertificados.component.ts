...
@Component({
  selector: 'app-AdmindetallesCertificados',
  templateUrl: './AdmindetallesCertificados.component.html',
  styleUrls: ['./AdmindetallesCertificados.component.css']
})
export class AdminCertificadosDetallesComponent implements OnInit {
  ...
  getSoticitud() {
    this.servicioStramite.get(this.route.snapshot.paramMap.get('detalleAdmin')).subscribe(solic => {
  ...
  getViewFile(url: string) {
    this.servicioDocumentos.getViewFile(url).subscribe(documento => {
      const file = URL.createObjectURL(documento);
  ...
  getUsuario() {
    ...
    this.funcionarios.id.toString();
    this.funcionarios.nombres.toString();
    this.funcionarios.apellidos.toString();
    this.funcionarios.rol.toString();
  ...
  filtarDocumentos() {
    ...
    for (let index = 0; index < this.documentos.length; index++) {
      ...
      if (element.estado != "ConceptoF" && element.codstramite == Number(this.route.snapshot.paramMap.get('detalleAdmin'))) {
  ...
  filtrarCertificados() {
    ...
    for (let index = 0; index < this.certificados.length; index++) {
      ...
      if (element.codstramite == Number(this.route.snapshot.paramMap.get('detalleAdmin')) && element.fechacreacion != "") {

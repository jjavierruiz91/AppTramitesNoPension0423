...
export class SolicitudesVistaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.tramite = { codtramite: 0, nombre: "", duracion: "", descripcion: "", costo: "", modalidad: "", url: "", sectorial: "" }
  ...
  filtarDocumentos() {
    ...
    for (let index = 0; index < this.documentos.length; index++) {
      ...
      if (element.codstramite == Number(this.route.snapshot.paramMap.get('idSoli'))) {
        // if (element.codstramite == Number(this.route.snapshot.paramMap.get('idSoli')&&element.estado != "ConceptoF") ) {
  ...
  update(): void {
    ...
    if (this.registerForm.invalid) {
      ...
      for (let index = 0; index < this.documentosFiltrados.length; index++) {
        ...
        if (element.estado != "Verificación exitosa." && element.estado != "ConceptoF") {
          contador++;
